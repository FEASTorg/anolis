...
namespace anolis
{
    namespace automation
    {
        ...
        bool ParameterDef::validate(const ParameterValue &new_value, std::string &error) const
        {
            ...
            {
                error = "Type mismatch: expected " + std::string(expected_type) +
            ...
            if (type == ParameterType::DOUBLE || type == ParameterType::INT64)
            {
                ...
                {
                    error = "Value " + std::to_string(numeric_value) +
                            " is below minimum " + std::to_string(min.value());
                    return false;
                }
                ...
                {
                    error = "Value " + std::to_string(numeric_value) +
                            " exceeds maximum " + std::to_string(max.value());
                    return false;
                }
            }
        ...
        bool ParameterManager::set(const std::string &name, const ParameterValue &value, std::string &error)
        {
...

            {
                ...
                if (old_val == new_val) changed = false; }, old_value);
