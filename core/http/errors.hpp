...
inline int status_code_to_http(StatusCode code) {
    switch (code) {
        ...
        case StatusCode::INVALID_ARGUMENT: return 400;
        case StatusCode::NOT_FOUND: return 404;
        case StatusCode::FAILED_PRECONDITION: return 409;
        case StatusCode::UNAVAILABLE: return 503;
        case StatusCode::DEADLINE_EXCEEDED: return 504;
...
inline nlohmann::json make_status(StatusCode code, const std::string& message = "") {
    std::string msg = message.empty() ? 
        (code == StatusCode::OK ? "ok" : status_code_to_string(code)) : message;
    return {
