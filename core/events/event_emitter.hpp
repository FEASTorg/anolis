    ...
    bool push(const Event& event) {
        ...
        if (queue_.size() >= max_size_) {
            ...
            if (dropped_count_ % 100 == 1) {
                std::cerr << "[EventEmitter] Queue '" << name_ 
                          << "' overflow, dropped " << dropped_count_ 
                          << " events total\n";
            }
        }
...
public:
    ...
    Subscription(Subscription&& other) noexcept
        : id_(other.id_), queue_(std::move(other.queue_)), 
     ...
     */
    size_t dropped_count() const {
        return queue_ ? queue_->dropped_count() : 0;
    }
    ...
    bool matches(const Event& event) const {
        return std::visit([this](auto&& e) -> bool {
            ...
            if (!provider_id.empty()) {
                if constexpr (std::is_same_v<T, StateUpdateEvent> || 
                              std::is_same_v<T, QualityChangeEvent> ||
...
public:
    ...
    std::unique_ptr<Subscription> subscribe(
        const EventFilter& filter = EventFilter::all(),
        size_t queue_size = 0,
        ...
        std::cerr << "[EventEmitter] Subscription " << id << " created"
                  << (name.empty() ? "" : " (" + name + ")")
                  << ", total subscribers: " << subscribers_.size() << "\n";
        
        auto unsubscribe_fn = [this](SubscriptionId sub_id) {
...
private:
    void unsubscribe(SubscriptionId id) {
        ...
        if (it != subscribers_.end()) {
            ...
            std::cerr << "[EventEmitter] Subscription " << id 
                      << " removed, remaining: " << subscribers_.size() << "\n";
        }
    }
