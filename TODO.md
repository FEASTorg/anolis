...
Decide and implement how every component of the system reaches an end user who
is not building from source. Covers all artifacts:

- [ ] **Runtime binary** (`anolis-runtime`): define release artifact format,
  versioning scheme, and target platforms (Linux ARM, Linux x86-64, Windows).
...
Do not design packaging around any specific delivery model until at least one
real end-to-end deployment on target hardware has been validated from source.
