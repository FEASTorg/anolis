...
TEST_F(StateCacheTest, TimeBasedStaleness) {
    ...
    // Capture the time "now" effectively used by the poll (it uses system_clock internally for setting timestamp during poll)
    // Actually, poll_once sets the timestamp = now().
    // We can't easily inject time into poll_once without refactoring StateCache::poll_once too, 
    // BUT we can use the time we pass to is_stale to simulate the passage of time relative to whenever the poll happened.
...
TEST_F(StateCacheTest, ConcurrencyStress) {
    ...
    std::thread writer([&]() {
        ...
        EXPECT_CALL(*mock_provider, read_signals("dev1", _, _))
            .WillRepeatedly(Invoke([](const std::string&, const std::vector<std::string>&, ReadSignalsResponse& response) {
                auto* v = response.add_values();
                v->set_signal_id("temp");
