    ...
    def run(self) -> ScenarioResult:
        ...
        try:
            ...
            result = self.call_function(
                "sim0", "tempctl0", "set_mode", {"mode": "closed"}
            )
            ...
            result = self.call_function(
                "sim0", "tempctl0", "set_mode", {"mode": "open"}
            )
            ...
            self.assert_equal(
                result["status"], "OK", "set_relay should succeed in open mode"
            )
            ...
            result = self.call_function(
                "sim0", "analogsensor0", "inject_noise", {"enabled": True}
            )
            self.assert_equal(
                result["status"], "OK", "Failed to enable noise injection"
            )
            ...
            result = self.call_function(
                "sim0", "analogsensor0", "inject_noise", {"enabled": False}
            )
            ...
            if quality_signal == "GOOD":
                result = self.call_function(
                    "sim0", "analogsensor0", "calibrate_channel", {"channel": 1}
                )
                self.assert_equal(
                    result["status"],
                    "OK",
            ...
            if quality_signal in ["NOISY", "FAULT"]:
                ...
                self.assert_true(
                    result.get("status")
                    in ["FAILED_PRECONDITION", "PRECONDITION_FAILED"],
